...
namespace ComputerGraphics.Tools
{
    class EnvBuilder
    {
        ...
        private void DrawLines(List<Line> lines, ref Matrix4x4 mvp)
        {
        ...
        private void DrawTriangles(List<Triangle> triangles, ref Matrix4x4 mvp)
        {
            foreach(Triangle triangle in triangles)
            {
                Vector4 pt0 = Matrix4x4.CrossProduct(mvp, new Vector4(triangle.pt0, 1));
                Vector4 pt1 = Matrix4x4.CrossProduct(mvp, new Vector4(triangle.pt1, 1));
                Vector4 pt2 = Matrix4x4.CrossProduct(mvp, new Vector4(triangle.pt2, 1));

                Triangle projectTriangle = new Triangle(pt0, pt1, pt2);
                if (projectTriangle.VisiableFromCamera(Vector3.Zero()))
                {
                    Point screenPt0 = ProjectToScreen(pt0);
                    Point screenPt1 = ProjectToScreen(pt1);
                    Point screenPt2 = ProjectToScreen(pt2);


                    DrawLineOnScreen(screenPt0, screenPt1);
                    DrawLineOnScreen(screenPt1, screenPt2);
                    DrawLineOnScreen(screenPt2, screenPt0);
                }
            }
        }
        private void Draw(Object3D ob)
        {
            ...
            if (ob.HasTriangle())
            {
                DrawTriangles(ob.GetTriangles(),ref mvp);
            }
            else
            {
                DrawLines(ob.GetLines(), ref mvp);
            }
        }
        ...
        {
            DrawLineOnScreen(screenFrom, screenTo, Color.Black);
        }
